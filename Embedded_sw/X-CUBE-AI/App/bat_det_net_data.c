...
ai_buffer ai_bat_det_net_data_activations_buffer_get(const ai_handle ptr)
{
  ai_buffer buf = AI_BUFFER_INIT(
    ...
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, AI_BAT_DET_NET_DATA_ACTIVATIONS_SIZE, 1, AI_BAT_DET_NET_DATA_ACTIVATIONS_COUNT),
    AI_BAT_DET_NET_DATA_ACTIVATIONS_SIZE,
    NULL, ptr);
  return buf;
}
...
ai_buffer ai_bat_det_net_data_weights_buffer_get(const ai_handle ptr)
{
  ai_buffer buf = AI_BUFFER_INIT(
    ...
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, AI_BAT_DET_NET_DATA_WEIGHTS_SIZE, 1, AI_BAT_DET_NET_DATA_WEIGHTS_COUNT),
    AI_BAT_DET_NET_DATA_WEIGHTS_SIZE,
    NULL, ptr);
  return buf;
}
...
{
  return AI_HANDLE_PTR(g_bat_det_net_weights_table);
